...
public class Main {
	...
	public void json() throws MFException, JSONException {
		...
		assertEquals(firstPart.getJSONArray("ea").getJSONObject(0).getDouble("percentage"),92.257895,0.01);
		
		// we change dataVersion
		options.put("referenceVersion","201507");
		json=JSONForMF.execute(options,groups,elements);
		assertEquals(json.getDouble("em")+"","1300.7825032239");
		...
		options.put("mf", "H2CO");
		json=JSONForMF.execute(options,groups,elements);
		assertEquals(json.getJSONArray("parts").getJSONObject(0).getDouble("unsaturation"),1.0,0);
		
		options.put("mf", "V");
		options.put("defaultUnsaturationContribution", "4");
		json=JSONForMF.execute(options,groups,elements);
		assertEquals(json.getJSONArray("parts").getJSONObject(0).getDouble("unsaturation"),3.0,0);
		
		options.put("mf", "CH4");
		json=JSONForMF.execute(options,groups,elements);
		assertEquals(json.getDouble("unsaturation"),0.0,0);
		...
		json=JSONForMF.execute(options,groups,elements);
		assertEquals(json.getDouble("unsaturation"),1.0,0);
		assertEquals(json.getDouble("charge"),-2.0,0);
		
		options.put("mf", "CO3(2-).2NH4+");
		json=JSONForMF.execute(options,groups,elements);
		assertEquals(json.getDouble("unsaturation"),1.0,0);
		assertEquals(json.getDouble("charge"),0.0,0);
		
		options.put("mf", "2CO3(2-).4NH4+");
		json=JSONForMF.execute(options,groups,elements);
		assertEquals(json.getDouble("unsaturation"),2.0,0);
		assertEquals(json.getDouble("charge"),0.0,0);
		
		options.put("mf", "2CO.3HO");
		json=JSONForMF.execute(options,groups,elements);
		// System.out.println(json);
		assertEquals(json.getJSONArray("ea").getJSONObject(0).getDouble("number"),2.0,0);
		assertEquals(json.getJSONArray("ea").getJSONObject(1).getString("element"),"O");
		assertEquals(json.getJSONArray("ea").getJSONObject(2).getDouble("percentage"),2.8248,0.001);
		
		// we change dataVersion and we check a non stable element
		options.put("referenceVersion","201507");
		options.put("mf", "Tc");
		json=JSONForMF.execute(options,groups,elements);
		assertEquals(json.getDouble("em")+"","97.907212436");
		assertEquals(json.getDouble("mw")+"","97.907212");
		assertEquals(json.getString("mf"),"Tc");

		options.put("mf", "Cm");
		json=JSONForMF.execute(options,groups,elements);
		// System.out.println(json);
		assertEquals(json.getDouble("em"),247.070354,0.0001);
		assertEquals(json.getDouble("mw"),247.070354147,0.0001);
		assertEquals(json.getString("mf"),"Cm");
	...
	public void isotopicDistribution() throws MFException, JSONException {
		...
		assertEquals(json.getDouble("mw"),1.2011E7,1000);
		...
		assertEquals(xy.indexOf("12010735.897, 100"),11386);
	}
	...
	public void isotopicDistributionRu() throws MFException, JSONException {
		...
		assertEquals(json.getDouble("em"),2038.08696,0.001);
		assertEquals(json.getDouble("mw"),2021.3,0.1);
		...
		assertEquals(xy.indexOf("2018.102, 100"),7731);
	}
	...
	public void mfFromMonosotopicMass() throws JSONException, MFException {
		...
		json=MFFromMonoisotopicMass.execute(options,groups,elements);
		assertEquals(json.getJSONArray("results").getJSONObject(0).getString("mf"),"C20H10(+2)");
		assertEquals(json.getJSONArray("results").getJSONObject(0).getDouble("error"),-5.487403E-4,0.00001);
	}
	...
	public void Charge() throws MFException, JSONException {
		...
		options.put("mf", "C(+10)");
		json=JSONForMF.execute(options,groups,elements);
		assertEquals(json.getJSONArray("parts").getJSONObject(0).getDouble("charge"),10.0,0.00001);
		...
		options.put("mf", "(C(---))2");
		json=JSONForMF.execute(options,groups,elements);
		assertEquals(json.getJSONArray("parts").getJSONObject(0).getDouble("charge"),-6.0,0.00001);
		assertEquals(json.getJSONArray("parts").getJSONObject(0).getString("mf"),"C2(-6)");
		
		options.put("mf", "((C(+3))2)3");
		json=JSONForMF.execute(options,groups,elements);
		assertEquals(json.getJSONArray("parts").getJSONObject(0).getDouble("charge"),18.0,0.00001);
		assertEquals(json.getJSONArray("parts").getJSONObject(0).getString("mf"),"C6(+18)");
	}
